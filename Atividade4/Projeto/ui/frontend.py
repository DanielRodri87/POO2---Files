...
from script.backend import Cliente, Cadastro 
...
class Ui(QtWidgets.QMainWindow):
    ...
    def verificar_cliente(self):
        ...
        if ok:
            ...
            if encontrado:
                cliente_info = self.cadastro.get_cliente_info(cpf)  # Obter o objeto Cliente completo
                if cliente_info:
                    nome_cliente = cliente_info.get_nome()
                    endereco_cliente = cliente_info.get_endereco()
                    cep_cliente = cliente_info.get_cep()
                    fone_cliente = cliente_info.get_fone()
                    QMessageBox.information(self, 'Cliente Encontrado',
                                            f'Cliente com CPF {cpf} foi {mensagem}.\n\n'
                                            f'Nome: {nome_cliente}\n'
                                            f'Endereço: {endereco_cliente}\n'
                                            f'CEP: {cep_cliente}\n'
                                            f'Telefone: {fone_cliente}')
            else:
